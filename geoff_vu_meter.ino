...
* https://www.hackster.io/ericBcreator/stereo-neopixel-ring-vu-meter-b28e78#code
**********************************************************************
*/
...
const int useSensorValues = false;                         // @EB

//
// setup pins
//

int leftPin = A0, rightPin = A2;                          // left audio in on analog 0, right on analog 1
int brightnessPin = A4, sensitivityPin = A5;              // potentiometers for brightness and sensitivity on analog 4 and 5
int leftStripPin = 6;                                     // DIN of left led strip on digital pin 5
int rightStripPin = 5;                                    // DIN of right led strip on digital pin 6
...
int useSelectButton1 = false;                              // set to false if no push button1 for selecting the color scheme is connected  @EB
...
int colorScheme = 1;                                     // 0: green-red, 1: blue-green, 2: blue-red, 3: red-blue, 4: green-blue, 5: red-green, 6: blue-white-red
...
int minValue = 17;                                        // min analog input value
...
#else
  int maxValue = 800;                                     // max analog input value (0-1023 equals 0-5V). try 300 for low level input, 700 for high
  int maxSensitivity = 8 * 255;                           // set to a higher setting to amplify low input levels. try 4 * 255 for low level input, 2 * 255 for high
#endif

int ledBrightness = 255;                                   // 0 - 255, initial value (value read from the potentiometer if useSensorValues = true)
int sensorDeviationBrightness = 10;                        // eliminate fluctuating values
...
int dynamicBouncingPeaks = true;                         // bounce less with lower peaks. note: bouncingPeaks has to be true 
